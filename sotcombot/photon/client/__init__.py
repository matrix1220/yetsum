...
class Bot:
	...
	async def _send(self, method, args={}):
		...
		print(method, "end", args, time.time()-time_)
	...
	async def long_polling(self, handle):
		offset = -1
		tmp = await self.getUpdates(offset=offset, allowed_updates=['message', 'callback_query'])
		for update in tmp:
			offset = update.update_id + 1

		while True:
			try:
				for update in await self.getUpdates(offset=offset, allowed_updates=['message', 'callback_query'], timeout=30):
					offset = update.update_id + 1
					try:
